...
const AccommodationPage = () => {
  ...
  const { checkIn, checkOut } = location.state || {}; // Get checkIn and checkOut dates
          ...
          {results.map(acc => (
            <div key={acc.id} className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col md:flex-row group">
              ...
              <div className="p-6 flex flex-col justify-between flex-1">
                ...
                {acc.isBooked ? (
                  <button
                    className="mt-auto flex items-center justify-center gap-2 w-full md:w-auto h-11 px-6 text-white text-sm font-bold rounded-lg transition-colors bg-gray-400 cursor-not-allowed"
                    disabled
                  >
                  ...
                  <Link
                    to={`/booking/${acc.id}`}
                    state={{ checkIn, checkOut }} // Pass checkIn and checkOut dates
                    className="mt-auto flex items-center justify-center gap-2 w-full md:w-auto h-11 px-6 text-white text-sm font-bold rounded-lg transition-colors bg-[#b2d7e5] hover:bg-[#a1c3d0]"
                  >
                    <span className="material-icons-outlined text-lg">shopping_cart</span>
                    Book Now
                  </Link>
